...
class AdminAuthenticator extends AbstractFormLoginAuthenticator implements PasswordAuthenticatedInterface
{
    ...
    public function getUser($credentials, UserProviderInterface $userProvider)
    {
        ...
        $user = $this->entityManager->getRepository(Admin::class)->findOneBy(['identifiant' => $credentials['identifiant']]);

        if (!$user) {
            // fail authentication with a custom error
            throw new CustomUserMessageAuthenticationException('Identifiants inconnu !');
        }
    ...
    {
      // SELECT mdp FROM `admin` WHERE `admin`.`identifiant` = 'b';

        $mdpSql = $user->getMdp();
        //dd($mdpSql,$credentials['mdp']);
        if($credentials['mdp'] === $mdpSql){
            return true;
        }
